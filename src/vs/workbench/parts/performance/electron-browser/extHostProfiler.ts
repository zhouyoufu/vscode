...
CommandsRegistry.registerCommand('exthost.profile.start', async accessor => {
  ...
  const handle = statusbarService.addEntry(
    { text: localize('message', '$(zap) Profiling Extension Host...') },
    StatusbarAlignment.LEFT
  )

  return TPromise.wrap(import('v8-inspect-profiler')).then(profiler => {
    return profiler
      .startProfiling({ port: extensionService.getExtensionHostInformation().inspectPort })
      .then(session => {
...
MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
  command: { id: 'exthost.profile.start', title: localize('', 'Profile Extension Host for 5 seconds') }
})
